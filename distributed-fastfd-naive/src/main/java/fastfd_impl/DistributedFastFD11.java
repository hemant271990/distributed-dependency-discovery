 ...
 * We calculate MaxSets here in hope of reducing computation.
 */
...
public class DistributedFastFD11 implements Serializable
{
	...
	public static HashMap<String, String> hashFuncs = new HashMap<String, String>();
	...
	public static long maxsetTime = 0;
	...
	public static void strippedPartitionGenerator() {
        ...
        Map<Integer, Map<Long, LongList>> spList = new HashMap<Integer, Map<Long, LongList>>();
		...
		for (int i = 0; i < numberAttributes; i++) {
			...
			Map<Long, LongList> lls = strippedPartitionPairRDD.collectAsMap();
			spList.put(i, lls);
		}
		...
		/* DEBUG */
		int sizeBefore = 0;
		for(Integer i : spList.keySet()) {
			//System.out.println(i+":");
			for(Long key : spList.get(i).keySet()){
				//System.out.println(key+" "+spList.get(i).get(key));
				sizeBefore += spList.get(i).get(key).size();
			}
		}
		System.out.println("Before pruning: "+ sizeBefore);
		
		long t3 = System.currentTimeMillis();
		HashMap<Integer, Map<Long, LongList>> maxSets = computeMaximumSets(spList);
		long t4 = System.currentTimeMillis();
		maxsetTime += t4 - t3;
		
		/* DEBUG */
		int sizeAfter = 0;
		for(Integer i : maxSets.keySet()) {
			//System.out.println(i+":");
			for(Long key : maxSets.get(i).keySet()){
				//System.out.println(key+" "+maxSets.get(i).get(key));
				sizeAfter += maxSets.get(i).get(key).size();
			}
		}
		System.out.println("\nAfter pruning: " + sizeAfter);
		
		long t5 = System.currentTimeMillis();
        for(Integer i : maxSets.keySet()) {
        	Map<Long, LongList> sp = maxSets.get(i);
        	Map<Long, Integer> hashFuncLoad = new HashMap<Long, Integer>();
        	
        	for(Long key : sp.keySet()) {
        		hashFuncLoad.put(key, sp.get(key).size());
        	}
        	...
        	for(Long key : sp.keySet()) {
    			for(Long e : sp.get(key)) {
    				hashFuncs.put("C"+i+"_"+e, "C"+i+"_"+key);
    			}
        	}
        }
	...
	public static HashMap<Integer, Map<Long, LongList>> computeMaximumSets(Map<Integer, Map<Long, LongList>> partitionsOrig) {
        HashMap<Integer, Map<Long, LongList>> maxSets = new HashMap<Integer, Map<Long, LongList>>();
        for(Integer key : partitionsOrig.keySet()) {
        	maxSets.put(key, new HashMap<Long, LongList>());
        	for(Long key2 : partitionsOrig.get(key).keySet()) {
        		maxSets.get(key).put(key2, partitionsOrig.get(key).get(key2));
        	}
        }
        /* DEBUG */
		/*System.out.println("\nIntermediate: ");
		for(Integer i : maxSets.keySet()) {
			for(Long key : maxSets.get(i).keySet()){
				System.out.println(maxSets.get(i).get(key));
			}
		}*/
        calculateSupersets(maxSets);
        return maxSets;
    }
	
	public static void calculateSupersets(Map<Integer, Map<Long, LongList>> maxSets) {
        Set<Long> toDelete = new HashSet<Long>();

        for(Integer key : maxSets.keySet()) {
	        for (Long key2 : maxSets.get(key).keySet()) {
	        	LongList maxSet = maxSets.get(key).get(key2);
	        	//System.out.println(key+" "+key2+" "+maxSet);
	        	boolean supersetFound = false;
	        	//System.out.println(maxSets.keySet());
	            for (Integer key3 : maxSets.keySet()) {
	            	//System.out.println(maxSets.get(key3).keySet());
	            	for(Long key4 : maxSets.get(key3).keySet()) {
	            		LongList partition = maxSets.get(key3).get(key4);
	            		//System.out.println("Checking against: "+ key3+" "+key4+" "+partition);
	            		if(maxSet.size() <= partition.size() && partition.containsAll(maxSet) && (key != key3 || key2 != key4)) {
	            			//System.out.println(" Found superSet: "+ key3+" "+key4+" "+partition);
	            			toDelete.add(key2);
	            			supersetFound = true;
	            			break;
	            		}
	        ...
	        for(Long keyToRemove : toDelete)
	        	maxSets.get(key).remove(keyToRemove);
	        toDelete = new HashSet<Long>();
        }
    }
...
//				System.out.println(fdResult.toString());
	...
	public static void execute() {
        ...
        System.out.println(" maxsetTime time(s): " + maxsetTime/1000);
