...
public abstract class EvidenceSetBuilder implements Serializable{
	...
	protected int compare(int i, int j, ColumnPair p) {
		return ((Comparable) getValue(i, p.getC1())).compareTo(((Comparable) getValue(j, p.getC2())));
	}

	protected boolean equals(int i, int j, ColumnPair p) {
